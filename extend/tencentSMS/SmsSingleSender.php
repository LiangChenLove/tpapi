<?php
namespace tencentSMS;

use tencentSMS\SmsSenderUtil;
...
class SmsSingleSender
{
	 ...
	 * @return array()
	 */
    public function saveSign($signName,$signid){
    	$url = 'https://yun.tim.qq.com/v5/tlssmssvr/mod_sign';
		$random = $this->util->getRandom();
		$curTime = time();
		...
		$data->time = $curTime;
		$data->sig = hash("sha256",
			"appkey=".$this->appkey."&random=".$random."&time=".$curTime, FALSE);
		return $this->util->sendCurlPost($wholeUrl, $data);
	}
	 ...
	 * @return array()
	 */
	public function addSign($signName){
		$url = 'https://yun.tim.qq.com/v5/tlssmssvr/add_sign';
		$random = $this->util->getRandom();
		$curTime = time();
		$wholeUrl = $url . "?sdkappid=" . $this->appid . "&random=" . $random;
	...
	public function getSign($signId){
		$url = 'https://yun.tim.qq.com/v5/tlssmssvr/get_sign';
		$random = $this->util->getRandom();
		$curTime = time();
		$wholeUrl = $url . "?sdkappid=" . $this->appid . "&random=" . $random;
